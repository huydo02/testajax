        ...
        <div class="modal-content">
            ...
            {{-- <div class="modal-body"> --}}
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="file" required class="course form-control" name="course">
                </div>
                <div class="form-group mb-3">
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        ...
        <form action="" method="POST" id="UpdateEmployeeForm" enctype="multipart/form-data">
                ...
                <div class="form-group mb-3">
                    <label for="">Full Name</label>
                    <input type="text" id="name_edt" name="name" required class="form-control">
                </div>
                <div class="form-group mb-3">
                    <label for="">image</label>
                    <input type="file" required class="course form-control" name="course">
                ...
                <div class="form-group mb-3">
                    <label for="">Email</label>
                    <input type="text" id="email_edt" name="email" required class="form-control">
                </div>
                <div class="form-group mb-3">
                    <label for="">Phone No</label>
                    <input type="text" id="phone_edt" name="phone" required class="form-control">
                </div>
            ...
            success: function (response) {
                ...
                $.each(response.students, function (key, item) {
                    $('tbody').append('<tr>\
                        <td>' + item.id + '</td>\
                        <td>' + item.name + '</td>\
                        <td><img src="uploads/studens/'+ item.course +'" width="50px" height="50px" alt="image"></td>\
                        <td>' + item.email + '</td>\
                        <td>' + item.phone + '</td>\
...
// edit student
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status == 404) {
                        ...
                        $('#name_edt').val(response.student.name);
                        // $('#course_edt').val(response.student.course);
                        $('#email_edt').val(response.student.email);
                        $('#phone_edt').val(response.student.phone);
                        $('#stud_id').val(stud_id);
                    }
                }
            });
    ...
    // $(document).ready(function () {
    //     //khi bat dau chay thi se trỏ đến hàm fetch này để show dữ liệu ra luôn
    //    
